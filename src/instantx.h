...
private:
    ...
    //track znodes who voted with no txreq (for DOS protection)
    std::map<COutPoint, int64_t> mapEnodeOrphanVotes; // mn outpoint - time
    ...
    int64_t GetAverageEnodeOrphanVoteTime();
...
private:
    uint256 txHash;
    COutPoint outpoint;
    COutPoint outpointEnode;
    std::vector<unsigned char> vchEnodeSignature;
    // local memory only
...
public:
    ...
    CTxLockVote(const uint256& txHashIn, const COutPoint& outpointIn, const COutPoint& outpointEnodeIn) :
        txHash(txHashIn),
        outpoint(outpointIn),
        outpointEnode(outpointEnodeIn),
        vchEnodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        READWRITE(txHash);
        READWRITE(outpoint);
        READWRITE(outpointEnode);
        READWRITE(vchEnodeSignature);
    }
    ...
    COutPoint GetEnodeOutpoint() const { return outpointEnode; }
...
private:
    ...
    std::map<COutPoint, CTxLockVote> mapEnodeVotes; // znode outpoint - vote
...
public:
    ...
    bool HasEnodeVoted(const COutPoint& outpointEnodeIn) const;
    int CountVotes() const { return mapEnodeVotes.size(); }
...
public:
    ...
    bool HasEnodeVoted(const COutPoint& outpointIn, const COutPoint& outpointEnodeIn);
