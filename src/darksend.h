...
// A helper object for signing messages from Enodes
...
public:
     ...
     *     1) we have an active Enode,
     *     2) we have a valid Enode private key,
     *     3) we signed the message successfully, and
     *     4) we verified the message successfully
     */
    bool Sign();
    /// Check if we have a valid Enode address
    bool CheckSignature(const CPubKey& pubKeyEnode);
...
{
public:
    /// Is the input associated with this public key? (and there is 1000 P2P - checking if valid znode)
...
private:
    ...
    // Keep track of the used Enodes
    std::vector<CTxIn> vecEnodesUsed;
    ...
    std::vector<CDarkSendEntry> vecEntries; // Enode/clients entries
    ...
    /// As a client, submit part of a future mixing transaction to a Enode to start the process
    ...
    /// Get Enode updates about the progress of mixing
