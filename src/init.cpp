...
void Shutdown() {
    ...
    // TODO: https://github.com/eledgerofficial/eledger/issues/182
    //    CFlatDB<CEnodeMan> flatdb1("zncache.dat", "magicEnodeCache");
    //    flatdb1.Dump(mnodeman);
    //    CFlatDB<CEnodePayments> flatdb2("znpayments.dat", "magicEnodePaymentsCache");
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if(torEnabledArg.second != "" && torEnabledArg.second != "0"){
        ...
        SetLimited(NET_IPV4);
        SetLimited(NET_IPV6);
        proxyType addrProxy = proxyType(CService("127.0.0.1", 12051),
                        true);
    ...
    if (proxyArg != "" && proxyArg != "0"){
        proxyType addrProxy = proxyType(CService(proxyArg, 12051), proxyRandomize);
    ...
    if (onionArg != "") {
        if (onionArg == "0") { // Handle -noonion/-onion=0
            ...
            proxyType addrOnion = proxyType(CService(onionArg, 12051), proxyRandomize);
...
//    nCoinCacheUsage = nTotalCache; // the rest goes to in-memory cache
    ...
    if ((fZNode || znodeConfig.getCount() > 0) && !fTxIndex) {
        return InitError("Enabling Enode support requires turning on transaction indexing."
    ...
    if (fZNode) {
        ...
        if (!GetArg("-znodeaddr", "").empty()) {
            // Hot Enode (either local or remote) should get its address in
            // CActiveEnode::ManageState() automatically and no longer relies on Enodeaddr.
            return InitError(_("znodeaddr option is deprecated. Please use znode.conf to manage your remote znodes."));
        }

        std::string strEnodePrivKey = GetArg("-znodeprivkey", "");
        if (!strEnodePrivKey.empty()) {
            if (!darkSendSigner.GetKeysFromSecret(strEnodePrivKey, activeEnode.keyEnode,
                                                  activeEnode.pubKeyEnode))
                return InitError(_("Invalid znodeprivkey. Please see documenation."));

            LogPrintf("  pubKeyEnode: %s\n", CBitcoinAddress(activeEnode.pubKeyEnode.GetID()).ToString());
        } else {
            return InitError(
    ...
    LogPrintf("Using Enode config file %s\n", GetEnodeConfigFile().string());

    if (GetBoolArg("-znconflock", true) && pwalletMain && (znodeConfig.getCount() > 0)) {
        LOCK(pwalletMain->cs_wallet);
        LogPrintf("Locking Enodes:\n");
        uint256 mnTxHash;
        int outputIndex;
...
//    nInstantSendDepth = std::min(std::max(nInstantSendDepth, 0), 60);
    ...
    // lite mode disables all Enode and Darksend related functionality
...
//    LogPrintf("PrivateSend amount %d\n", nPrivateSendAmount);
    ...
    if (mnodeman.size()) {
        uiInterface.InitMessage(_("Loading Enode payment cache..."));
        CFlatDB<CEnodePayments> flatdb2("znpayments.dat", "magicEnodePaymentsCache");
        if (!flatdb2.Load(mnpayments)) {
            return InitError("Failed to load znode payments cache from znpayments.dat");
        }
    } else {
        uiInterface.InitMessage(_("Enode cache is empty, skipping payments and governance cache..."));
    } */
