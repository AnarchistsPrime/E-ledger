    ...
    if ((nHeight + 1 > 0) && (nHeight + 1 < 210000)) {
        ...
        if (nHeight < Params().GetConsensus().nEnodePaymentsStartBlock) {
            ...
            if (!fTestNet && (GetAdjustedTime() > nStartRewardTime)) {
                ...
                if (nHeight + 1 < 14000) {
                    FOUNDER_2_SCRIPT = GetScriptForDestination(CBitcoinAddress("a8wUrPhGYUAoYtFR2TNn9usErhpwJcHjPE").Get());
                } else {
                    FOUNDER_2_SCRIPT = GetScriptForDestination(CBitcoinAddress("a8wUrPhGYUAoYtFR2TNn9usErhpwJcHjPE").Get());
                }
        ...
        } else if (nHeight >= Params().GetConsensus().nEnodePaymentsStartBlock) {
            // Take some reward away from us
...
//            }
        ...
        if (nHeight >= chainparams.GetConsensus().nEnodePaymentsStartBlock) {
            CAmount znodePayment = GetEnodePayment(nHeight, blockReward);
            coinbaseTx.vout[0].nValue -= znodePayment;
            FillBlockPayments(coinbaseTx, nHeight, znodePayment, pblock->txoutEnode, pblock->voutSuperblock);
        }
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock, true, NULL, false))
        return error("EledgerMiner: ProcessNewBlock, block not accepted");

    return true;
}

void static EledgerMiner(const CChainParams &chainparams) {
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("eledger-miner");
    ...
    try {
        ...
        if (!coinbaseScript || coinbaseScript->reserveScript.empty()) {
            LogPrintf("EledgerMiner stop here coinbaseScript=%s, coinbaseScript->reserveScript.empty()=%s\n", coinbaseScript, coinbaseScript->reserveScript.empty());
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("Error in EledgerMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running EledgerMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//                        CheckWork(pblock, *pwallet, reservekey);
        ...
        LogPrintf("EledgerMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e) {
        LogPrintf("EledgerMiner runtime error: %s\n", e.what());
        return;
    }
}
...
void GenerateBitcoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EledgerMiner, boost::cref(chainparams)));
}
