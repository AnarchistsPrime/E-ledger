...
CNode *ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, bool fConnectToEnode) {
    if (pszDest == NULL) {
        // we clean znode connections in CEnodeMan::ProcessEnodeConnections()
        // so should be safe to skip this and connect to local Hot MN on CActiveEnode::ManageState()
        if (IsLocal(addrConnect) && !fConnectToEnode)
            return NULL;
        LOCK(cs_vNodes);
        ...
        if (pnode) {
            ...
            if (fConnectToEnode && !pnode->fEnode) {
                pnode->fEnode = true;
            }
            pnode->AddRef();
            return pnode;
