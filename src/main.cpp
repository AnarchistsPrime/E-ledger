...
#if defined(NDEBUG)
# error "Eledger cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Eledger Signed Message:\n";
...
//                                      !(pindexBIP34height->GetBlockHash() == chainparams.GetConsensus().BIP34Hash));
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (txIds.count(txHash) > 0 && (fTestNet || pindex->nHeight >= HF_ENODE_HEIGHT))
    ...
    // ENODE : MODIFIED TO CHECK ENODE PAYMENTS AND SUPERBLOCKS
...
CAmount GetEnodePayment(int nHeight, CAmount blockValue) {
//    CAmount ret = blockValue * 30/100 ; // start at 30%
//    int nMNPIBlock = Params().GetConsensus().nEnodePaymentsStartBlock;
////    int nMNPIBlock = Params().GetConsensus().nEnodePaymentsIncreaseBlock;
//    int nMNPIPeriod = Params().GetConsensus().nEnodePaymentsIncreasePeriod;
//
//    // mainnet:
...
bool CheckBlock(const CBlock &block, CValidationState &state, const Consensus::Params &consensusParams, bool fCheckPOW,
    ...
    try {
        ...
        if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
            ...
            BOOST_FOREACH(const CTransaction& tx, block.vtx) {
                ...
                BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                    ...
                    if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                        ...
                        LOCK(cs_main);
                        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                        return state.DoS(0, error("CheckBlock(P2P): transaction %s conflicts with transaction lock %s",
            ...
            LogPrintf("CheckBlock(P2P): spork is off, skipping transaction locking checks\n");
        }
...
bool static AlreadyHave(const CInv &inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    */
        ...
        case MSG_ENODE_PAYMENT_VOTE:
            return mnpayments.mapEnodePaymentVotes.count(inv.hash);

        case MSG_ENODE_PAYMENT_BLOCK:
        {
            BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
            return mi != mapBlockIndex.end() && mnpayments.mapEnodeBlocks.find(mi->second->nHeight) != mnpayments.mapEnodeBlocks.end();
        }

        case MSG_ENODE_ANNOUNCE:
            return mnodeman.mapSeenEnodeBroadcast.count(inv.hash) && !mnodeman.IsMnbRecoveryRequested(inv.hash);

        case MSG_ENODE_PING:
            return mnodeman.mapSeenEnodePing.count(inv.hash);

        case MSG_DSTX:
            return mapDarksendBroadcastTxes.count(inv.hash);

        case MSG_ENODE_VERIFY:
            return mnodeman.mapSeenEnodeVerification.count(inv.hash);
    }
...
//                LogPrintf("inv.type()=%s, inv.GetCommand=%s\n", inv.type, inv.GetCommand());
                ...
                if (!pushed && inv.type == MSG_ENODE_PAYMENT_VOTE) {
                    if(mnpayments.HasVerifiedPaymentVote(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnpayments.mapEnodePaymentVotes[inv.hash];
                        pfrom->PushMessage(NetMsgType::ENODEPAYMENTVOTE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_ENODE_PAYMENT_BLOCK) {
                    BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
                    LOCK(cs_mapEnodeBlocks);
                    if (mi != mapBlockIndex.end() && mnpayments.mapEnodeBlocks.count(mi->second->nHeight)) {
                        BOOST_FOREACH(CEnodePayee& payee, mnpayments.mapEnodeBlocks[mi->second->nHeight].vecPayees) {
                            std::vector<uint256> vecVoteHashes = payee.GetVoteHashes();
                            BOOST_FOREACH(uint256& hash, vecVoteHashes) {
                                if(mnpayments.HasVerifiedPaymentVote(hash)) {
                                    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                                    ss.reserve(1000);
                                    ss << mnpayments.mapEnodePaymentVotes[hash];
                                    pfrom->PushMessage(NetMsgType::ENODEPAYMENTVOTE, ss);
                                }
                            }
                        }
                ...
                if (!pushed && inv.type == MSG_ENODE_ANNOUNCE) {
                    if(mnodeman.mapSeenEnodeBroadcast.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenEnodeBroadcast[inv.hash].second;
                        pfrom->PushMessage(NetMsgType::MNANNOUNCE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_ENODE_PING) {
                    if(mnodeman.mapSeenEnodePing.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenEnodePing[inv.hash];
                ...
                if (!pushed && inv.type == MSG_ENODE_VERIFY) {
                    if(mnodeman.mapSeenEnodeVerification.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenEnodeVerification[inv.hash];
...
bool static ProcessMessage(CNode *pfrom, string strCommand, CDataStream &vRecv, int64_t nTimeReceived,
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        int minPeerVersion = (nHeight + 1 < HF_ENODE_HEIGHT) ? MIN_PEER_PROTO_VERSION : MIN_PEER_PROTO_VERSION_AFTER_ENODE_PAYMENT_HF;
...
//        vRecv >> tx;
        ...
        if (strCommand == NetMsgType::TXLOCKREQUEST) {
            ...
            CEnode *pmn = mnodeman.Find(dstx.vin);
            if (pmn == NULL) {
            ...
            if (!pmn->fAllowMixingTx) {
                LogPrint("privatesend", "DSTX -- Enode %s is sending too many transactions %s\n",
                         dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return true;
            ...
            if (!dstx.CheckSignature(pmn->pubKeyEnode)) {
