...
void CDarksendPool::ProcessMessage(CNode *pfrom, std::string &strCommand, CDataStream &vRecv) {
    ...
    if (strCommand == NetMsgType::DSACCEPT) {
        ...
        if (!fZNode) {
            LogPrintf("DSACCEPT -- not a Enode!\n");
        ...
        CEnode *pmn = mnodeman.Find(activeEnode.vin);
        if (pmn == NULL) {
        ...
        CEnode *pmn = mnodeman.Find(dsq.vin);
        if (pmn == NULL) return;

        if (!dsq.CheckSignature(pmn->pubKeyEnode)) {
        ...
        if (dsq.fReady) {
            if (!pSubmittedToEnode) return;
            if ((CNetAddr) pSubmittedToEnode->addr != (CNetAddr) pmn->addr) {
                LogPrintf("DSQUEUE -- message doesn't match current Enode: pSubmittedToEnode=%s, addr=%s\n", pSubmittedToEnode->addr.ToString(), pmn->addr.ToString());
                return;
            }
            ...
            q, vecDarksendQueue) {
                if (q.vin == dsq.vin) {
                    ...
                    LogPrint("privatesend", "DSQUEUE -- Enode %s is sending WAY too many dsq messages\n", pmn->addr.ToString());
                    return;
                }
            }
            ...
            if (pmn->nLastDsq != 0 && nThreshold > mnodeman.nDsqCount) {
                LogPrint("privatesend", "DSQUEUE -- Enode %s is sending too many dsq messages\n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            if (pSubmittedToEnode && pSubmittedToEnode->vin.prevout == dsq.vin.prevout) {
                dsq.fTried = true;
            }
        ...
        if (fZNode) {
            // LogPrintf("DSSTATUSUPDATE -- Can't run on a Enode!\n");
            return;
        }

        if (!pSubmittedToEnode) return;
        if ((CNetAddr) pSubmittedToEnode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSSTATUSUPDATE -- message doesn't match current Enode: pSubmittedToEnode %s addr %s\n", pSubmittedToEnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (!fZNode) {
            LogPrintf("DSSIGNFINALTX -- not a Enode!\n");
            return;
        }
        ...
        if (fZNode) {
            // LogPrintf("DSFINALTX -- Can't run on a Enode!\n");
            return;
        }

        if (!pSubmittedToEnode) return;
        if ((CNetAddr) pSubmittedToEnode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSFINALTX -- message doesn't match current Enode: pSubmittedToEnode %s addr %s\n", pSubmittedToEnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (fZNode) {
            // LogPrintf("DSCOMPLETE -- Can't run on a Enode!\n");
            return;
        }

        if (!pSubmittedToEnode) return;
        if ((CNetAddr) pSubmittedToEnode->addr != (CNetAddr) pfrom->addr) {
            LogPrint("privatesend", "DSCOMPLETE -- message doesn't match current Enode: pSubmittedToEnode=%s  addr=%s\n", pSubmittedToEnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
//
// Check the mixing progress and send client updates if a Enode
//
...
void CDarksendPool::CommitFinalTransaction() {
    ...
    if (!mapDarksendBroadcastTxes.count(hashTx)) {
        CDarksendBroadcastTx dstx(finalTransaction, activeEnode.vin, GetAdjustedTime());
        dstx.Sign();
...
//
// How does this work? Messages to Enodes come in via NetMsgType::DSVIN, these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the Enode
...
void CDarksendPool::CheckForCompleteQueue() {
    ...
    if (nState == POOL_STATE_QUEUE && IsSessionReady()) {
        ...
        CDarksendQueue dsq(nSessionDenom, activeEnode.vin, GetTime(), true);
...
//
// Execute a mixing denomination via a Enode.
// This is only ran from clients
//
bool CDarksendPool::SendDenominate(const std::vector <CTxIn> &vecTxIn, const std::vector <CTxOut> &vecTxOut) {
    if (fZNode) {
        LogPrintf("CDarksendPool::SendDenominate -- PrivateSend from a Enode is not supported currently.\n");
        return false;
    }
    ...
    // we should already be connected to a Enode
    if (!nSessionID) {
        LogPrintf("CDarksendPool::SendDenominate -- No Enode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return false;
    }
...
// Incoming message from Enode updating the progress of mixing
bool CDarksendPool::CheckPoolStateUpdate(PoolState nStateNew, int nEntriesCountNew, PoolStatusUpdate nStatusUpdate, PoolMessage nMessageID, int nSessionIDNew) {
    ...
    strAutoDenomResult = _("Enode:") + " " + GetMessageByID(nMessageID);

    // if rejected at any state
    if (nStatusUpdate == STATUS_REJECTED) {
        LogPrintf("CDarksendPool::CheckPoolStateUpdate -- entry is rejected by Enode\n");
        UnlockCoins();
        SetNull();
        SetState(POOL_STATE_ERROR);
...
//
// After we receive the finalized transaction from the Enode, we must
...
bool CDarksendPool::SignFinalTransaction(const CTransaction &finalTransactionNew, CNode *pnode) {
    ...
    // push all of our signatures to the Enode
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun) {
    ...
    if (mnodeman.size() == 0) {
        LogPrint("privatesend", "CDarksendPool::DoAutomaticDenominating -- No Enodes detected\n");
        strAutoDenomResult = _("No Enodes detected.");
        return false;
    }
    ...
    // If we've used 90% of the Enode list then drop the oldest first ~30%
    int nThreshold_high = nMnCountEnabled * 0.9;
    int nThreshold_low = nThreshold_high * 0.7;
    LogPrint("privatesend", "Checking vecEnodesUsed: size: %d, threshold: %d\n", (int) vecEnodesUsed.size(), nThreshold_high);

    if ((int) vecEnodesUsed.size() > nThreshold_high) {
        vecEnodesUsed.erase(vecEnodesUsed.begin(), vecEnodesUsed.begin() + vecEnodesUsed.size() - nThreshold_low);
        LogPrint("privatesend", "  vecEnodesUsed: new size: %d, threshold: %d\n", (int) vecEnodesUsed.size(), nThreshold_high);
    }
    ...
    if (nLiquidityProvider || fUseQueue) {
        ...
        BOOST_FOREACH(CDarksendQueue & dsq, vecDarksendQueue)
        {
            ...
            vecEnodesUsed.push_back(dsq.vin);
            ...
            // connect to Enode and submit the queue request
            CNode *pnode = (pnodeFound && pnodeFound->fEnode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
            if (pnode) {
                pSubmittedToEnode = pmn;
                ...
                strAutoDenomResult = _("Error connecting to Enode.");
                continue;
            }
        }
    }
    ...
    while (nTries < 10) {
        CEnode *pmn = mnodeman.FindRandomNotInVec(vecEnodesUsed, MIN_PRIVATESEND_PEER_PROTO_VERSION);
        if (pmn == NULL) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Can't find random znode!\n");
            strAutoDenomResult = _("Can't find random Enode.");
            return false;
        }
        vecEnodesUsed.push_back(pmn->vin);
        ...
        LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt %d connection to Enode %s\n", nTries, pmn->addr.ToString());
        CNode *pnode = (pnodeFound && pnodeFound->fEnode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
        if (pnode) {
    ...
    strAutoDenomResult = _("No compatible Enode found.");
    return false;
}
...
bool CDarksendPool::CreateNewSession(int nDenom, CTransaction txCollateral, PoolMessage &nMessageIDRet) {
    ...
    if (!fUnitTest) {
        ...
        CDarksendQueue dsq(nDenom, activeEnode.vin, GetTime(), false);
...
std::string CDarksendPool::GetMessageByID(PoolMessage nMessageID) {
    switch (nMessageID) {
        ...
        case ERR_MN_LIST:
            return _("Not in the Enode list.");
        case ERR_MODE:
            return _("Incompatible mode.");
        case ERR_NON_STANDARD_PUBKEY:
            return _("Non-standard public key detected.");
        case ERR_NOT_A_MN:
            return _("This is not a Enode.");
        case ERR_QUEUE_FULL:
            return _("Enode queue is full.");
        case ERR_RECENT:
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn &txin, const CPubKey &pubkey) {
    ...
    if (GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        ...
        if (out.nValue == ENODE_COIN_REQUIRED * COIN && out.scriptPubKey == payee) return true;
    }
...
bool CDarksendQueue::Sign() {
    ...
    return CheckSignature(activeEnode.pubKeyEnode);
}

bool CDarksendQueue::CheckSignature(const CPubKey &pubKeyEnode) {
    std::string strMessage = vin.ToString() + boost::lexical_cast<std::string>(nDenom) + boost::lexical_cast<std::string>(nTime) + boost::lexical_cast<std::string>(fReady);
    std::string strError = "";

    if (!darkSendSigner.VerifyMessage(pubKeyEnode, vchSig, strMessage, strError)) {
        LogPrintf("CDarksendQueue::CheckSignature -- Got bad Enode queue signature: %s; error: %s\n", ToString(), strError);
        return false;
    }
...
bool CDarksendBroadcastTx::CheckSignature(const CPubKey &pubKeyEnode) {
    std::string strMessage = tx.GetHash().ToString() + boost::lexical_cast<std::string>(sigTime);
    std::string strError = "";
...
void CDarksendPool::RelayIn(const CDarkSendEntry &entry) {
    ...
    CNode *pnode = FindNode(pSubmittedToEnode->addr);
    if (pnode != NULL) {
...
void CDarksendPool::SetState(PoolState nStateNew) {
    if (fZNode && (nStateNew == POOL_STATE_ERROR || nStateNew == POOL_STATE_SUCCESS)) {
        LogPrint("privatesend", "CDarksendPool::SetState -- Can't set state to ERROR or SUCCESS as a Enode. \n");
        return;
    }
...
void CDarksendPool::UpdatedBlockTip(const CBlockIndex *pindex) {
    ...
    if (!fLiteMode && znodeSync.IsEnodeListSynced()) {
        NewBlock();
    }
}
...
void ThreadCheckDarkSendPool() {
    ...
    while (true) {
        ...
        if (znodeSync.IsBlockchainSynced() && !ShutdownRequested()) {
            ...
            if (nTick % ENODE_MIN_MNP_SECONDS == 15)
                activeEnode.ManageState();

            if (nTick % 60 == 0) {
                mnodeman.ProcessEnodeConnections();
