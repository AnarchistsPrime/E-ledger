...
//    if(!sporkManager.IsSporkActive(SPORK_2_INSTANTSEND_ENABLED)) return;
    ...
    // Ignore any InstantSend messages until enode list is synced
...
bool CInstantSend::ProcessTxLockRequest(const CTxLockRequest& txLockRequest)
{
    ...
    // Enodes will sometimes propagate votes before the transaction is known to the client.
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        int n = mnodeman.GetEnodeRank(activeEnode.vin, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

        if(n == -1) {
            LogPrint("instantsend", "CInstantSend::Vote -- Unknown Enode %s\n", activeEnode.vin.prevout.ToStringShort());
            ++itOutpointLock;
            continue;
        }
        ...
        if(n > nSignaturesTotal) {
            LogPrint("instantsend", "CInstantSend::Vote -- Enode not in the top %d (%d)\n", nSignaturesTotal, n);
            ++itOutpointLock;
            continue;
        }
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            BOOST_FOREACH(const uint256& hash, itVoted->second) {
                ...
                if(it2->second.HasEnodeVoted(itOutpointLock->first, activeEnode.vin.prevout)) {
        ...
        CTxLockVote vote(txHash, itOutpointLock->first, activeEnode.vin.prevout);
    ...
    if(it == mapTxLockCandidates.end()) {
        if(!mapTxLockVotesOrphan.count(vote.GetHash())) {
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  znode=%s new\n",
                    txHash.ToString(), vote.GetEnodeOutpoint().ToStringShort());
        ...
        int nEnodeOrphanExpireTime = GetTime() + 60*10; // keep time data for 10 minutes
        if(!mapEnodeOrphanVotes.count(vote.GetEnodeOutpoint())) {
            mapEnodeOrphanVotes[vote.GetEnodeOutpoint()] = nEnodeOrphanExpireTime;
        } else {
            int64_t nPrevOrphanVote = mapEnodeOrphanVotes[vote.GetEnodeOutpoint()];
            if(nPrevOrphanVote > GetTime() && nPrevOrphanVote > GetAverageEnodeOrphanVoteTime()) {
    ...
    if(it1 != mapVotedOutpoints.end()) {
        BOOST_FOREACH(const uint256& hash, it1->second) {
            if(hash != txHash) {
                ...
                std::map<uint256, CTxLockCandidate>::iterator it2 = mapTxLockCandidates.find(hash);
                if(it2->second.HasEnodeVoted(vote.GetOutpoint(), vote.GetEnodeOutpoint())) {
...
int64_t CInstantSend::GetAverageEnodeOrphanVoteTime()
{
    LOCK(cs_instantsend);
    // NOTE: should never actually call this function when mapEnodeOrphanVotes is empty
    if(mapEnodeOrphanVotes.empty()) return 0;

    std::map<COutPoint, int64_t>::iterator it = mapEnodeOrphanVotes.begin();
    int64_t total = 0;

    while(it != mapEnodeOrphanVotes.end()) {
        total+= it->second;
        ++it;
    }

    return total / mapEnodeOrphanVotes.size();
}
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsExpired(pCurrentBlockIndex->nHeight)) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired vote: txid=%s  znode=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetEnodeOutpoint().ToStringShort());
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(GetTime() - itOrphanVote->second.GetTimeCreated() > ORPHAN_VOTE_SECONDS) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan vote: txid=%s  znode=%s\n",
                    itOrphanVote->second.GetTxHash().ToString(), itOrphanVote->second.GetEnodeOutpoint().ToStringShort());
    ...
    std::map<COutPoint, int64_t>::iterator itEnodeOrphan = mapEnodeOrphanVotes.begin();
    while(itEnodeOrphan != mapEnodeOrphanVotes.end()) {
        if(itEnodeOrphan->second < GetTime()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan znode vote: znode=%s\n",
                    itEnodeOrphan->first.ToStringShort());
            mapEnodeOrphanVotes.erase(itEnodeOrphan++);
        } else {
            ++itEnodeOrphan;
        }
    }
...
{
    if(!mnodeman.Has(CTxIn(outpointEnode))) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Unknown znode %s\n", outpointEnode.ToStringShort());
        mnodeman.AskForMN(pnode, CTxIn(outpointEnode));
        return false;
    }
    ...
    int n = mnodeman.GetEnodeRank(CTxIn(outpointEnode), nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);
    ...
    LogPrint("instantsend", "CTxLockVote::IsValid -- Enode %s, rank=%d\n", outpointEnode.ToStringShort(), n);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(n > nSignaturesTotal) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Enode %s is not in the top %d (%d), vote hash=%s\n",
                outpointEnode.ToStringShort(), nSignaturesTotal, n, GetHash().ToString());
        return false;
    }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    znode_info_t infoMn = mnodeman.GetEnodeInfo(CTxIn(outpointEnode));

    if(!infoMn.fInfoValid) {
        LogPrintf("CTxLockVote::CheckSignature -- Unknown Enode: znode=%s\n", outpointEnode.ToString());
        return false;
    }

    if(!darkSendSigner.VerifyMessage(infoMn.pubKeyEnode, vchEnodeSignature, strMessage, strError)) {
...
bool CTxLockVote::Sign()
{
    ...
    if(!darkSendSigner.SignMessage(strMessage, vchEnodeSignature, activeEnode.keyEnode)) {
        LogPrintf("CTxLockVote::Sign -- SignMessage() failed\n");
        return false;
    }

    if(!darkSendSigner.VerifyMessage(activeEnode.pubKeyEnode, vchEnodeSignature, strMessage, strError)) {
...
{
    if(mapEnodeVotes.count(vote.GetEnodeOutpoint()))
        return false;
    mapEnodeVotes.insert(std::make_pair(vote.GetEnodeOutpoint(), vote));
    return true;
}
...
std::vector<CTxLockVote> COutPointLock::GetVotes() const
{
    ...
    std::map<COutPoint, CTxLockVote>::const_iterator itVote = mapEnodeVotes.begin();
    while(itVote != mapEnodeVotes.end()) {
...
bool COutPointLock::HasEnodeVoted(const COutPoint& outpointEnodeIn) const
{
    return mapEnodeVotes.count(outpointEnodeIn);
}
...
bool CTxLockCandidate::HasEnodeVoted(const COutPoint& outpointIn, const COutPoint& outpointEnodeIn)
{
    std::map<COutPoint, COutPointLock>::iterator it = mapOutPointLocks.find(outpointIn);
    return it !=mapOutPointLocks.end() && it->second.HasEnodeVoted(outpointEnodeIn);
}
