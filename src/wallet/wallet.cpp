...
//                continue;
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fZNode && pcoin->vout[i].nValue == ENODE_COIN_REQUIRED * COIN);
                } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if (found && fZNode) found = pcoin->vout[i].nValue != ENODE_COIN_REQUIRED * COIN; // do not use Hot MN funds
                } else if (nCoinType == ONLY_1000) {
                    found = pcoin->vout[i].nValue == ENODE_COIN_REQUIRED * COIN;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector <CTxIn> &vecTxInRet, CAmount &nValueRet,
    ...
    BOOST_FOREACH(const COutput &out, vCoins)
    {
        ...
        if (fZNode && out.tx->vout[out.i].nValue == ENODE_COIN_REQUIRED * COIN) continue; //znode input
...
bool CWallet::GetEnodeVinAndKeys(CTxIn &txinRet, CPubKey &pubKeyRet, CKey &keyRet, std::string strTxHash,
    ...
    if (vPossibleCoins.empty()) {
        LogPrintf("CWallet::GetEnodeVinAndKeys -- Could not locate any valid znode vin\n");
        return false;
    }
    ...
    LogPrintf("CWallet::GetEnodeVinAndKeys -- Could not locate specified znode vin\n");
    return false;
}
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if (fAnonymizable) {
                ...
                if (fZNode && wtx.vout[i].nValue == ENODE_COIN_REQUIRED * COIN) continue;
...
//                bool fFirst = true;
                ...
                if (!SelectCoins(vAvailableCoins, nValueToSelect, setCoins, nValueIn, coinControl)) {
                    if (nCoinType == ONLY_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 P2P.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough PrivateSend non-denominated funds for this transaction that are not equal 1000 P2P.");
...
//                bool fAllowFree = false;					// No free TXs in P2P
