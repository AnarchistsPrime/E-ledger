...
//            "  \"znode_payments_enforced\" : true|false, (boolean) true, if znode payments are enforced\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Eledger is not connected!");
    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Eledger is downloading blocks...");

    if (!znodeSync.IsSynced())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Eledger Core is syncing with network...");
...
//    const bool fPreSegWit = (THRESHOLD_ACTIVE != VersionBitsState(pindexPrev, consensusParams, Consensus::DEPLOYMENT_SEGWIT, versionbitscache));
    ...
    BOOST_FOREACH (CTransaction& tx, pblock->vtx) {
        ...
        // https://github.com/eledgerofficial/eledger/pull/26
    ...
    if(pblock->txoutEnode != CTxOut()) {
        CTxDestination address1;
        ExtractDestination(pblock->txoutEnode.scriptPubKey, address1);
        CBitcoinAddress address2(address1);
        znodeObj.push_back(Pair("payee", address2.ToString().c_str()));
        znodeObj.push_back(Pair("script", HexStr(pblock->txoutEnode.scriptPubKey.begin(), pblock->txoutEnode.scriptPubKey.end())));
        znodeObj.push_back(Pair("amount", pblock->txoutEnode.nValue));
    }
    result.push_back(Pair("znode", znodeObj));
    result.push_back(Pair("znode_payments_started", pindexPrev->nHeight + 1 > Params().GetConsensus().nEnodePaymentsStartBlock));
