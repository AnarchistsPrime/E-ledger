...
//    obj.push_back(Pair("mixing_mode",       fPrivateSendMultiSession ? "multi-session" : "normal"));
    ...
    if (darkSendPool.pSubmittedToEnode) {
        obj.push_back(Pair("outpoint", darkSendPool.pSubmittedToEnode->vin.prevout.ToStringShort()));
        obj.push_back(Pair("addr", darkSendPool.pSubmittedToEnode->addr.ToString()));
    }
...
UniValue znode(const UniValue &params, bool fHelp) {
    ...
    if (strCommand == "connect") {
        if (params.size() < 2)
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Enode address required");
    ...
    if (strCommand == "count") {
        ...
        int nCount;
        mnodeman.GetNextEnodeInQueueForPayment(true, nCount);
    ...
    if (strCommand == "current" || strCommand == "winner") {
        ...
        winner = mnodeman.GetNextEnodeInQueueForPayment(nHeight, true, nCount);
        ...
        obj.push_back(Pair("lastseen", (winner->lastPing == CEnodePing()) ? winner->sigTime :
                                       winner->lastPing.sigTime));
        obj.push_back(Pair("activeseconds", (winner->lastPing == CEnodePing()) ? 0 :
    ...
    if (strCommand == "debug") {
        if (activeEnode.nState != ACTIVE_ENODE_INITIAL || !znodeSync.IsBlockchainSynced())
            return activeEnode.GetStatus();

        CTxIn vin;
        CPubKey pubkey;
        CKey key;

        if (!pwalletMain || !pwalletMain->GetEnodeVinAndKeys(vin, pubkey, key))
    ...
    if (strCommand == "start") {
        ...
        if (activeEnode.nState != ACTIVE_ENODE_STARTED) {
            activeEnode.nState = ACTIVE_ENODE_INITIAL; // TODO: consider better way
    ...
    if (strCommand == "start-alias") {
        ...
        BOOST_FOREACH(CEnodeConfig::CEnodeEntry mne, znodeConfig.getEntries()) {
            if (mne.getAlias() == strAlias) {
                ...
                bool fResult = CEnodeBroadcast::Create(mne.getIp(), mne.getPrivKey(), mne.getTxHash(),
...
//        LogPrintf("start-alias: statusObj=%s\n", statusObj);
    ...
    if (strCommand == "start-all" || strCommand == "start-missing" || strCommand == "start-disabled") {
        ...
        if ((strCommand == "start-missing" || strCommand == "start-disabled") &&
            !znodeSync.IsEnodeListSynced()) {
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
                               "You can't use this command until enode list is synced");
        }
    ...
    if (strCommand == "status") {
        ...
        mnObj.push_back(Pair("vin", activeEnode.vin.ToString()));
        mnObj.push_back(Pair("service", activeEnode.service.ToString()));

        CEnode mn;
        if (mnodeman.Get(activeEnode.vin, mn)) {
            mnObj.push_back(Pair("payee", CBitcoinAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
        }

        mnObj.push_back(Pair("status", activeEnode.GetStatus()));
        return mnObj;
    }
...
UniValue znodelist(const UniValue &params, bool fHelp) {
    ...
    UniValue obj(UniValue::VOBJ);
    if (strMode == "rank") {
        std::vector <std::pair<int, CEnode>> vEnodeRanks = mnodeman.GetEnodeRanks();
        BOOST_FOREACH(PAIRTYPE(int, CEnode) & s, vEnodeRanks)
        {
...
bool DecodeHexVecMnb(std::vector <CEnodeBroadcast> &vecMnb, std::string strHexMnb) {
...
UniValue znodebroadcast(const UniValue &params, bool fHelp) {
    ...
    if (strCommand == "create-alias") {
        ...
        UniValue statusObj(UniValue::VOBJ);
        std::vector <CEnodeBroadcast> vecMnb;

        statusObj.push_back(Pair("alias", strAlias));

        BOOST_FOREACH(CEnodeConfig::CEnodeEntry
    ...
    if (strCommand == "create-all") {
        ...
        std::vector <CEnodeConfig::CEnodeEntry> mnEntries;
    ...
    if (strCommand == "decode") {
        ...
        if (!DecodeHexVecMnb(vecMnb, params[1].get_str()))
            throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Enode broadcast message decode failed");
        ...
        BOOST_FOREACH(CEnodeBroadcast & mnb, vecMnb)
        {
            UniValue resultObj(UniValue::VOBJ);
            ...
            if (mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyEnode", CBitcoinAddress(mnb.pubKeyEnode.GetID()).ToString()));
                ...
                nFailed++;
                resultObj.push_back(Pair("errorMessage", "Enode broadcast signature verification failed"));
            }
    ...
    if (strCommand == "relay") {
        ...
        BOOST_FOREACH(CEnodeBroadcast & mnb, vecMnb)
        {
            ...
            if (mnb.CheckSignature(nDos)) {
                if (fSafe) {
                    fResult = mnodeman.CheckMnbAndUpdateEnodeList(NULL, mnb, nDos);
                } else {
                    mnodeman.UpdateEnodeList(mnb);
                    mnb.RelayZNode();
                    fResult = true;
