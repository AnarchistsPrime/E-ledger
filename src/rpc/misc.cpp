...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"eledgeraddress\"\n"
            "\nReturn information about the given eledger address.\n"
            "\nArguments:\n"
            "1. \"eledgeraddress\"     (string, required) The eledger address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"eledgeraddress\", (string) The eledger address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: eledger address and we have full public key:
...
UniValue znsync(const UniValue& params, bool fHelp)
{
    ...
    if(strMode == "status") {
        ...
        objStatus.push_back(Pair("IsEnodeListSynced", znodeSync.IsEnodeListSynced()));
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are eledger addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) eledger address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"eledgeraddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"eledgeraddress\"  (string, required) The eledger address to use for the signature.\n"
